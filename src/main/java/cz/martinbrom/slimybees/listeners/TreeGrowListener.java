...
public class TreeGrowListener implements Listener {
        ...
        if (isValidTreeType(world, e.getSpecies())) {
            ...
            try (EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder()
            	...
            	for (NestDTO nest : nests) {
                    if (tryGenerateNest(editSession, b, nest)) {
                        return;
                    }
                }
    ...
    private boolean tryGenerateNest(EditSession editSession, Block b, NestDTO nest) {
        ...
        for (BlockFace blockFace : BLOCK_FACES) {
            ...
            if (nestBlock.getType().isAir() && groundBlock.getType().isSolid()) {
                NestPopulator.createNest(editSession, nestBlock, nest);
